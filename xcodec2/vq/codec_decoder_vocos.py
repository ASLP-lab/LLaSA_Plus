...
class VocosBackbone(Backbone):
    ...
    def forward(self, x: torch.Tensor, past_key_values=None, use_cache=False) -> torch.Tensor:
        x = x.transpose(1, 2)
        x = self.embed(x)
        x = self.prior_net(x)
        x = x.transpose(1, 2)
        if past_key_values is None:
            past_key_values = tuple([None] * len(self.transformers))
        # import pdb;pdb.set_trace()
        presents = () if use_cache else None
        if use_cache:
            for i, layer in enumerate(self.transformers):
                layer_past = past_key_values[i]
                x , kvcache = layer(x, layer_past, use_cache)

                presents = presents + (kvcache,)
        else:
...
class CodecDecoderVocos(nn.Module):
    ...
    def forward(self, x, vq=True, past_key_values=None, use_cache=False, hidden_history=None):
        if vq is True:
        ...
        #torch.Size([1, 300, 1024]) #torch.float32
        if use_cache:
            x, past_key_values = self.backbone(x, past_key_values, use_cache)
        else:
            x = self.backbone(x, past_key_values, use_cache)
        # import pdb;pdb.set_trace()
        if hidden_history is not None: #拼接先前的history hidden_state
            # x = torch.concat((x, hidden_history),dim=1)
            pass
        hidden_history = x
        #torch.Size([1, 25, 1024]) #torch.float32
        x,_  = self.head(x)
        ...
        # 分块测试逻辑
        if test_chunks:
            rec_full= x.squeeze().detach().cpu().numpy()
            import soundfile as sf
            sr = 16000
            sf.write('full.flac',rec_full,sr)
            chunk_sizes = [80, 100, 120, 140, 160, 180, 200, 220, 240]
            import pdb;pdb.set_trace()
            for chunk_size in chunk_sizes:
                chunk = hidden_history[:,:chunk_size,:]
                
                chunk_audio, _ = self.head(chunk)

                chunk_audio = chunk_audio.squeeze().detach().cpu().numpy()
                
                chunk_audio = chunk_audio[:16000] #只保存1s
                tmp = chunk_audio
                
                sf.write(f'chunk_{chunk_size}.flac', chunk_audio, sr)
